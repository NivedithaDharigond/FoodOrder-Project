...
export const fetchCartItems = (alert) => async (dispatch) => {
  ...
  export const addItemToCart =
    (foodItemId, restaurant, quantity, alert) => async (dispatch,getState) => {
      try {
        //getstate returns  the current state tree of ur application, which is equal to the last value returned by the stored reducer
        const { user } = getState().auth;
        const response = await axios.post("/api/v1/eats/cart/add-to-cart", {
          //particular account which fooditem is selected, from which restaurant
          //and how much quantity is selected
          userId: user._id,
          foodItemId,
          restaurantId: restaurant,
          quantity,
        });
        alert.success("Item is added to Cart ", response.data.cart);
        dispatch({
          type: ADD_TO_CART,
